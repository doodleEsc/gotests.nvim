...
local function smartrun()
  ...
  if has_main() then
    ...
    cmd = string.format("lcd %:p:h | :set makeprg=%s\\ run\\ . | :make | :lcd -", "go")
    vim.cmd(cmd)
  else
    cmd = string.format("setl makeprg=%s\\ run\\ . | :make", "go")
    vim.cmd(cmd)
  end
end
...
local function smartbuild()
  ...
  if has_main() then
    -- Found main function in current buffer
    cmd = string.format("lcd %:p:h | :set makeprg=%s\\ build\\ . | :make | :lcd -", "go")
    vim.cmd(cmd)
  else
    cmd = string.format("setl makeprg=%s\\ build\\ . | :make", "go")
    vim.cmd(cmd)
  end
end
...
-- util.log = function(...)
--   if not require("gotests").GO_NVIM_CFG.verbose then
--     return
--   end
--   local arg = { ... }
--
...
function util.check_capabilities(feature, client_id)
  ...
  if supported_client then
    ...
    if #clients == 0 then
      util.warn("LSP: no client attached")
    else
      util.warn("LSP: server does not support " .. feature)
    end
    return false
  end
end
